...
function Leaderboard({ go, dbReady }) {
  ...
  useEffect(() => {
    if (!dbReady) {
      const demo = [...FAKE_LEADERBOARD].map((r, i) => ({ nome: r.name, gironi: r.gironi, secche: r.secche, tot: r.gironi + r.secche, verificata: i % 2 === 0 }));
    ...
    (async () => {
      try {
        ...
        const scored = preds.map((p) => {
          ...
          return { nome: p.nome, gironi: s.gironi, secche: s.secche, tot: s.tot, verificata: !!p.verificata };
        });
